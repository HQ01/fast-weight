...
def _gru_convolution(X, n_filters, weight):
...
def _gru(X, settings, parameters, memory):
  ...
  if previous_h is 0:
    ...
    array = _gru_convolution(X, n_filters, X_weight) + _gru_convolution(previous_h, n_filters, h_weight)
    array = layers.broadcast_plus(array, bias)
...
def _write(X, settings, parameters, memory):
  memory = _gru(X, settings, parameters, memory)
  return memory

_n_gru_attention_module = 0
def _gru_attention_module(network, settings):
  global _n_gru_attention_module
  prefix = 'gru_attention_module%d' % _n_gru_attention_module
  ...
  gru_bias = layers.variable('%s_gru_bias' % prefix, shape=(1, 4 * n_filters, 1, 1))
  gru_parameters = (X_weight, h_weight, gru_bias)
  memory = (0, 0)
  ...
  for index in range(settings['n_layers']):
    memory = _write(network, memory_settings, gru_parameters, memory) # dynamic period of memory writing
    network = _read(memory_settings, memory)
    network = _normalized_convolution(X=network, **kwargs)
#   network = _normalized_convolution(X=network, **kwargs)

  _n_gru_attention_module += 1
  return network
...
def gru_attention_network(settings):
  network = layers.variable('data')
  network = layers.batch_normalization(network)
  for module_settings in settings:
    if module_settings['operator'] is 'gru_attention_module':
      network = _gru_attention_module(network, module_settings['settings'])
...
if __name__ is '__main__':
  ...
  network = gru_attention_network(settings)
