...
for index in range(configs.n_plain_layers):
  network = _normalized_convolution(X=network, n_filters=16, kernel_shape=(3, 3), stride=(1, 1), pad=(1, 1), weight=weight)
...
identifier = 'shrinked-mnist-shared-weight-plain-network-%d-%s' % (configs.n_plain_layers, configs.postfix)
