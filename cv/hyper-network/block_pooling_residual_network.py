...
def _recur(network, module_index, settings):
  ...
  n_filters = {0 : 16, 1 : 32, 2 : 64}[module_index]
  refining_times = settings['refining_times'][module_index]
  if module_index == 0: refining_times += 1 # identical to original paper
  for time in range(refining_times):
    residual = _normalized_weighted_convolution(network, (3, 3), n_filters, (1, 1), (1, 1))
    residual = _normalized_weighted_convolution(residual, (3, 3), n_filters, (1, 1), (1, 1))
    network = network + residual
  pooling_times = settings['pooling_times'][module_index]
  for time in range(pooling_times):
    residual = _normalized_unweighted_convolution(network, (3, 3), n_filters, (1, 1), (1, 1))
    residual = _normalized_unweighted_convolution(residual, (3, 3), n_filters, (1, 1), (1, 1))
    network = network + residual
  return network

def pooled_residual_network(settings):
