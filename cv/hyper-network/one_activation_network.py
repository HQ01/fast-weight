...
def one_activation_network(n_layers):
  ...
  for i in range(n_layers):
    private_weight = layers.variable('%d_weight' % i)
    private_bias = layers.variable('%d_bias' % i)
    weight = private_weight + shared_weight
    bias = private_bias + shared_bias
    network = _normalized_convolution(network, (3, 3), 16, (1, 1), (1, 1), weight=weight, bias=bias)
